...
def unstack_bert_features(input_ids, input_mask, segment_ids, bert_module):
    input_ids = tf.reshape(input_ids, [-1, doclen])
    input_mask = tf.reshape(input_mask, [-1, doclen])
    segment_ids = tf.reshape(segment_ids, [-1, doclen])
...
def init_input(doclen, embsize):
    ...
    with tf.name_scope("inputs"):
        x = tf.placeholder(tf.float32, [INPUT_DIMS, max_sent, 768], name='input_X')
        input_ids = tf.placeholder(tf.int32, [INPUT_DIMS, max_sent, doclen], name='input_ids')
        input_masks = tf.placeholder(tf.int32, [INPUT_DIMS, max_sent, doclen], name='input_masks')
        segment_ids = tf.placeholder(tf.int32, [INPUT_DIMS, max_sent, doclen], name='segment_ids')
