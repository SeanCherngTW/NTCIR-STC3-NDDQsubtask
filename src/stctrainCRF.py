...
def start_trainND(
        ...
        testX, testND, test_turns, test_masks,
        epoch, early_stopping, batch_size, lr, kp, hiddens, Fsize, Fnum, gating, bn, num_layers, method, evaluate, memory_rnn_type=None, bert=None,
):
    assert method.__name__ in ['CNNRNN', 'CNNCNN']

    tf.reset_default_graph()
    x, bertx, y, bs, turns, masks, num_sent = ND.init_input(doclen, embsize)

    if bert:
        log_likelihood, pred = method(bertx, y, bs, turns, kp, hiddens, Fsize, Fnum, gating, bn, num_layers, masks, memory_rnn_type, bert)
    else:
        log_likelihood, pred = method(x, y, bs, turns, kp, hiddens, Fsize, Fnum, gating, bn, num_layers, masks, memory_rnn_type, bert)
    with tf.name_scope('loss'):
        cost = ND.CRF_loss_function(log_likelihood, num_sent)
    ...
    with tf.Session(config=config).as_default() as sess:
        ...
        for e in range(epoch):
            ...
            for i in range(0, len_train, batch_size):
                ...
                sess.run(train_op, feed_dict={bertx: trainX[i:j], y: trainY[i:j], bs: train_bs,
            ...
            for i in range(0, len_train, batch_size):
                ...
                train_loss += sess.run(cost, feed_dict={bertx: trainX[i:j], y: trainY[i:j], bs: train_bs,
                                                        turns: train_turns[i:j], masks: train_masks[i:j], num_sent: train_num_sent, })
            ...
            for i in range(0, len_dev, batch_size):
                ...
                dev_loss += sess.run(cost, feed_dict={bertx: devX[i:j], y: devND[i:j], bs: dev_bs,
            ...
            if current_early_stoping >= early_stopping or (e + 1) == epoch:
                ...
                pred_dev = sess.run(pred, feed_dict={bertx: devX, bs: len_dev, turns: dev_turns, masks: dev_masks})
                dev_confusion_matrix, dev_acc = STCE.nugget_evaluation_CRF(pred_dev, devND)

                args = ['ND', str(memory_rnn_type), 'CNN{}'.format(len(Fnum)), 'RNN{}'.format(num_layers), "{:.4f}".format(dev_acc)]
                argstr = '_'.join(map(str, args))
                print(argstr)
                with open(argstr + '_dev.csv', 'w') as f:
                    for row in dev_confusion_matrix:
                        row = list(map(str, row))
                        f.write(','.join(row) + '\n')
                break

        if evaluate:
            pred_test = sess.run(pred, feed_dict={bertx: testX, bs: len(testX), turns: test_turns, masks: test_masks})
            test_confusion_matrix, test_acc = STCE.nugget_evaluation_CRF(pred_test, testND)
            with open(argstr + '_test.csv', 'w') as f:
