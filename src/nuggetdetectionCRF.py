...
def init_input(doclen, embsize):
    ...
    with tf.name_scope("inputs"):
        ...
        bertx = tf.placeholder(tf.float32, [None, max_sent, 1024], name='input_bertX')
        y = tf.placeholder(tf.int32, [None, max_sent], name='output_Y')
        bs = tf.placeholder(tf.int32, [], name='batch_size')
        turns = tf.placeholder(tf.int32, [None, ], name='turns')
        masks = tf.placeholder(tf.int32, [None, max_sent, NDclasses], name='masks')
        num_sent = tf.placeholder(tf.int32, [], name='num_sent')
    return x, bertx, y, bs, turns, masks, num_sent
...
def CRF_loss_function(log_likelihood, num_sent):
    return tf.reduce_sum(-log_likelihood) / tf.cast(num_sent, tf.float32)
...
def CNNRNN(x, y, bs, turns, keep_prob, rnn_hiddens, filter_size, num_filters, gating, batch_norm, num_layers, masks, memory_rnn_type=None, bert=None):
    ...
    # fc_outputs = tf.cast(fc_outputs, tf.int32)
    y = tf.cast(y, tf.int32)

    log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(fc_outputs, y, turns)
    viterbi_sequence, viterbi_score = tf.contrib.crf.crf_decode(fc_outputs, transition_params, turns)
