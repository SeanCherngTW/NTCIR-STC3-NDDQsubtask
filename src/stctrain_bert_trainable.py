...
BERT_PICKLE_PATH = 'PickleBertTrainable/'


def get_data():
    trainX_input_ids = pickle.load(open(BERT_PICKLE_PATH + 'trainX_input_ids.p', 'rb'))
    trainX_input_masks = pickle.load(open(BERT_PICKLE_PATH + 'trainX_input_masks.p', 'rb'))
    trainX_segment_ids = pickle.load(open(BERT_PICKLE_PATH + 'trainX_segment_ids.p', 'rb'))
    devX_input_ids = pickle.load(open(BERT_PICKLE_PATH + 'devX_input_ids.p', 'rb'))
    devX_input_masks = pickle.load(open(BERT_PICKLE_PATH + 'devX_input_masks.p', 'rb'))
    devX_segment_ids = pickle.load(open(BERT_PICKLE_PATH + 'devX_segment_ids.p', 'rb'))
    testX_input_ids = pickle.load(open(BERT_PICKLE_PATH + 'testX_input_ids.p', 'rb'))
    testX_input_masks = pickle.load(open(BERT_PICKLE_PATH + 'testX_input_masks.p', 'rb'))
    testX_segment_ids = pickle.load(open(BERT_PICKLE_PATH + 'testX_segment_ids.p', 'rb'))
...
def unstack_bert_features(input_ids, input_mask, segment_ids, bert_module):
    ...
    for diaID, (dialog_ids, dialog_masks, dialog_segids) in enumerate(zip(input_ids, input_mask, segment_ids)):
        for uttID, (_id, mask, segid) in enumerate(zip(dialog_ids, dialog_masks, dialog_segids)):
            bert_inputs = dict(
                input_ids=[_id],
                input_mask=[mask],
                segment_ids=[segid],
            ...
            pooled_output = bert_outputs["pooled_output"]  # (1, 768)
            if uttID == 0:
                utt_pooled_outputs = pooled_output
            else:
                utt_pooled_outputs = tf.concat([utt_pooled_outputs, pooled_output], axis=0)

        utt_pooled_outputs = tf.expand_dims(utt_pooled_outputs, axis=0)

        if diaID == 0:
            dia_pooled_outputs = utt_pooled_outputs
        else:
            dia_pooled_outputs = tf.concat([dia_pooled_outputs, utt_pooled_outputs], axis=0)
    print('FINAL bert out shape {}'.format(dia_pooled_outputs.shape))
    return dia_pooled_outputs
...
def start_trainND(
    ...
    pred = method(x, input_ids, input_masks, segment_ids, y, bs, turns, kp, hiddens, Fsize, Fnum, gating, bn, num_layers, masks, memory_rnn_type)
    ...
    with tf.Session(config=config).as_default() as sess:
        ...
        for e in range(epoch):
            ...
            for i in range(0, len_train, batch_size):
                ...
                print('End batch {}-{}/{}'.format(i, j, len_train))
            ...
            for i in range(0, len_dev, batch_size):
                ...
                dev_loss += sess.run(cost, feed_dict={input_ids: devX_input_ids[i:j], input_masks: devX_input_masks[i:j], segment_ids: devX_segment_ids[i:j], y: devND[i:j], bs: dev_bs,
